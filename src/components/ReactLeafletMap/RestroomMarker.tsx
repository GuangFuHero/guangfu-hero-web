...
const RestroomMarker = ({ restroom, isVisible }: RestroomMarkerProps) => {
  ...
  return (
    <Marker position={[lat, lng]} icon={restroomIcon} dataId={restroom.id}>
      <Popup>
        <div className="max-w-xs">
          <h3 className="font-bold text-lg mb-2">{restroom.name || '-'}</h3>
          <div className="space-y-1 text-sm">
            <div>
              <span className="font-semibold">地址:</span> {restroom.address || '-'}
            </div>
            {restroom.facilities && (
              <div>
                <span className="font-semibold">設施:</span> {restroom.facilities}
              </div>
            )}
            {restroom.notes && (
              <div>
          ...
          <div className="mt-3 pt-2 border-t border-gray-200">
            <button
              onClick={() =>
                openReportModal('廁所', restroom.id || 'unknown', restroom.name || '未知廁所')
              }
